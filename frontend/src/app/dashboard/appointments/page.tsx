...
import type { PaginatedResponse, Appointment } from '@/types'
...
const STATUS_COLORS: Record<string, string> = {
  ...
  completed: 'bg-[#1E1E35] text-[#64748B] border border-[#2A2A45]',
}
...
export default function AppointmentsPage() {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ['appointments', channel, status, page],
    queryFn: () =>
      get<PaginatedResponse<Appointment>>(
        `/dashboard/appointments?page=${page}&page_size=20${channel !== 'all' ? `&channel=${channel}` : ''}${status !== 'all' ? `&status=${status}` : ''}`
      ),
  })

  const updateMutation = useMutation({
    mutationFn: ({ id, status }: { id: string; status: string }) =>
      patch(`/dashboard/appointments/${id}`, { status }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['appointments'] })
      toast.success('Appointment updated')
    },
    onError: (e) => toast.error('Failed to update appointment', (e as Error).message),
  })

  const appointments = data?.data ?? []
  const filtered = search
    ? appointments.filter((a) =>
        [a.customer_name, a.customer_phone, a.customer_email, a.service_type, a.appointment_date]
          .filter(Boolean)
  ...
  const exportCSV = () => {
    ...
    const headers = ['Name', 'Phone', 'Email', 'Channel', 'Service', 'Date', 'Time', 'Status', 'Captured']
    const rows = appointments.map((a) => [
      a.customer_name ?? '', a.customer_phone ?? '', a.customer_email ?? '',
      a.channel, a.service_type ?? '', a.appointment_date ?? '', a.appointment_time ?? '',
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-lg font-semibold text-[#F1F1F5]">Appointments</h2>
          <p className="text-xs text-[#64748B] mt-0.5">
            {data?.total ?? 0} total appointments booked
          </p>
        </div>
      ...
      <div className="bg-[#16162A] border border-[#1E1E35] rounded-xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full" role="table" aria-label="Appointments table">
            <thead>
              <tr className="border-b border-[#1E1E35]">
                {['Contact', 'Channel', 'Service', 'Date & Time', 'Status', 'Captured', 'Actions'].map((h) => (
                  <th
                    key={h}
                    scope="col"
            ...
            <tbody className="divide-y divide-[#1E1E35]/50">
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={7} className="px-4 py-12 text-center">
                    <div className="flex flex-col items-center gap-2">
                      <p className="text-sm text-[#4A4A6A]">Failed to load appointments</p>
                ...
                <tr>
                  <td colSpan={7} className="px-4 py-12 text-center">
                    <div className="flex flex-col items-center gap-2">
                      <Calendar size={24} className="text-[#2A2A45]" />
                      <p className="text-sm text-[#4A4A6A]">No appointments yet</p>
                      <p className="text-xs text-[#4A4A6A]">Appointments are captured when customers book via chat</p>
                    </div>
                  </td>
                </tr>
              ) : (
                filtered.map((appt, i) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <span className={`inline-flex items-center text-[10px] font-medium px-2 py-0.5 rounded-full capitalize ${STATUS_COLORS[appt.status]}`}>
                        {appt.status}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <span className="text-xs text-[#64748B]">
                        {appt.created_at ? formatRelativeTime(appt.created_at) : '—'}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <select
                        value={appt.status}
                        onChange={(e) =>
                          updateMutation.mutate({ id: appt.id, status: e.target.value })
                        }
                        ...
                        aria-label={`Update status for ${appt.customer_name ?? appt.id}`}
                      >
                        {STATUS_OPTIONS.map((s) => (
                          <option key={s} value={s} className="capitalize">{s}</option>
                        ))}
